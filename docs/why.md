...
```py
  ...
  def split(i, t, br):
    ...
    for col in t.cols.x:
      if  ins(Col.nums, col.txt):
        for pos1, lo1, hi1, s in i.nums(t,col,br):
          if s > most:
            pos, lo, hi, most = pos1, lo1, hi1, s
      else:
        for pos1, lo1, hi1, s in i.syms(t,col,br):
          if s > most:
            pos, lo, hi, most = pos1, lo1, hi1, s
    return pos, lo, hi
...
```py
  def syms(i, t, col, br):
    ...
    for k,b in br.best.cols[col.pos].seen.items():
      if b > 0:
        r  = br.rest.cols[col.pos].seen.get(k,0)
        b  = b/bs
        r  = r/rs
        if b>r:
