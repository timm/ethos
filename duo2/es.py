...
def es(
    ...
    COPYRIGHT: "show copyright" = FALSE,
    ...
    SIZE     : "split n numbers into bins of size n**DIV" = 0.7, 
    VERSION  : "show version" = False,
    DOC      : "Internal. documentation string. Do not change" = [],
    ARGS     : "Internal . Do not change" = [],
  }
  """ x"es.py : catch sight of (something that is partially hidden, or obscure)
  __WHO__ = "Tim Menzies, timm@ieee.org"
  __COPY__ = "MIT 3 clauselicense"
  __VERSION__ = 0.14
  """
  # ------------------------------------------
  # ## Column Classes
 ...
 # start up actions
 lib.main(ARG=ARG,DOC=DOC,funs=vars(tests))
...
class lib:
 ...
 #start up actions
  def main(ARG=[],DOC="",funs=[]):
    for n,c in enumerate(ARG):
      if   c == "-h": print(DOC                     
      elif c == "-C": print(DOC__.split("\n\n")[0])  
      elif c == "-t": [lib.run(funs[k]) for k in funs if a[n+1] in k]
      elif c == "-T": lib.banner();  [lib.run(funs[k]) for k in funs]
      elif c == "-L": 
        [print(k,"\t:",v.__doc__) for k,v in funs.items() if type(v)==fun] 
      elif c[0] == "-" : 
        print("usage: ./keys.py -[hCt:T] [args]")
    sys.exit(0 if lib.fails==1 else 1)# While spurious adjacent bins exist, merge them.
 ...
 # es(ARG=sys.argv,  DOC=es.__doc__)
