...
class Sym(Col):
  ...
  def spurious(i, j, goal):
    ...
    if modei == modej:
      ...
      for x,n in i.seen.items(): k.seen[x] = k.seen.get(x,0) + n
      for x,n in j.seen.items(): k.seen[x] = k.seen.get(x,0) + n
      for x,n in k.seen.items():
        if n > k.max:
           k.mode, k.max = x,n
      return k
...
def counts(t, goal,  **kw):
  ...
  for col in t.cols.x:
    if type(col) == Sym:
      col.bins = col.bins or [Span(down=x,up=x) for x in col.seen.keys]
    else:
      col.bins = col.bins or div(t, col, **kw)
      merge(col.bins) 
