...
class Tab(o):
  def __init__(i, keep=1024, div=.5,opt=None, all=[]):
...
def unsuper(t, col, size=.75, cohen=.35):
  ...
  for j,(x,y) in enumerate(xy):
    if j < len(xy) - width:
      if now.also.n >= width:
        if x != xy[j+1][0] and now.up - now.down > col.sd()*cohen:
          now  = Span(now.up, x)
          out += [now]
    now.up = x
    now.also + y
...
# Merge spurious adjacent bins; i.e. those 
# that predict for the same label.
# Generate frequency counts of how often the merged bins
# appear in `goal` and `not goal`.
class Discretize(o):
  def __init__(i, goal,tab,size=.75, cohen=.35): 
    i.f, i.h, i.n = {}, {}, {}
    i.tab, i.goal = tab, goal
    for col in tab.cols.x:
      if type(col) == Sym:
        col.bins = col.bins or [Span(down=x,up=x) for x in col.all.keys()]
        i.count(col, col.bins)
      else:
        col.bins = col.bins or unsuper(t, col, size=size, cohen=cohen)
        i.count(col, merge(col.bins))

  # Update our frequency counts
  def count(i, col, bins):
    def inc(d,k): d[k] = d.get(k,0) + 1
    f, h, n = i.f, i.h, i,n
    for row in i.tab.rows.all:
      x = row.cells[col.pos]
      if x != the.ch.no:
        for b in bins:
          if b.has(x):
  ...
  def merge(i,b4):
    ...
    while j < n:
      ...
      if j < n - 1:
        b  = b4[j+1]
        now = a.also.spurious(b.also, goal)
        if now:
          a = Span(a.down, b.up)
          a.also = now
          j += 1
...
class Optimize(o):
  def __init__(i, tab, div=0.5):
   i.tab, i.div = tab, div
   # for n,rows in enumerate(t.clusters()): 
   #   for row in rows: row.tag = n
  def clusters(i): 
    gt = lambda a,b: 0 if id(a)==id(b) else (-1 if a.better(b, i.tab.cols) else 1)
    rows = i.tab.rows.all
    rows = sorted(rows, key=functools.cmp_to_key(gt))
    return lib.chunks(rows, 2*len(rows)**i.div)
